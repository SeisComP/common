...
void sc_locsat_rdtttab(
	...
	float tempval,      /* Temporary read before assign       */
	    *temp_tbz, *temp_tbd, *temp_tt, *tflt;

	char filnam[1024];        /* ["root" + "." + "postname" + NULL] */
	const char *const *s2ptr; /* Temporary array pointer */
	...
	cnt = 0;
	num_files = 0; /* Counters */

	/* Read the files */

	for ( err = 0, s2ptr = phase_type_ptr, k = 0; k < nwav && !err;
	      ++k, ++s2ptr ) {
		strcpy(filnam, froot);
		strcat(filnam, ".");
		strcat(filnam, *s2ptr);

		/** HACK to avoid problems with filesystems not able to handle
		...
		*/
		...
		if ( !err && (ntbz[k] = ntbzx) > maxtbz ) {
			fprintf(
			    stderr, "%s %s, %d used, %d skipped\n",
			    "\nToo many depth samples in file", filnam, maxtbz,
			    ntbzx - maxtbz
			);
			ntbz[k] = maxtbz;
			err = 3;
		...
		if ( !err && (ntbd[k] = ntbdx) > maxtbd ) {
			fprintf(
			    stderr, "%s %s, %d used, %d skipped\n",
			    "\nToo many distance samples in file", filnam, maxtbd,
			    ntbdx - maxtbd
			);
			ntbd[k] = maxtbd;
			err = 3;
