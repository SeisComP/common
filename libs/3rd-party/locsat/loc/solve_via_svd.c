...
void sc_locsat_solve_via_svd(
	...
	int i, icnt, info, j, job, k, neig, norder;
	double *e, *g, *gscale, *gtr, smax, sum, *sval;
	double *tmp, *u, *v, *work;
	...
	for ( i = 0; i < nd; ++i ) {
		for ( j = 0; j < np; ++j ) {
			g[j + i * MAX_PARAM] = data[i].at[j] * gscale[j];
		}
	}
	...
	gtrnorm = 0.0;
	for ( i = 0; i < nd; ++i ) {
		rnorm = rnorm + data[i].resid2 * data[i].resid2;
		for ( j = 0; j < np; ++j ) {
			dscale = g[j + i * MAX_PARAM];
			frob += dscale * dscale;
		}
	}
	...
	for ( j = 0; j < np; ++j ) {
		gtr[j] = 0.0;
		for ( i = 0; i < nd; ++i ) {
			gtr[j] = gtr[j] + g[j + i * MAX_PARAM] * data[i].resid2;
		}
...
rank_of_matrix:
	...
	for ( i = 0; i < np; ++i ) {
		for ( j = 0; j <= i; ++j ) {
			...
			icnt = info + 1 + (neig - 1);
			for ( k = info; k < icnt; ++k ) {
				sum += u[i + k * MAX_PARAM] * u[j + k * MAX_PARAM] * sval[k];
			}
