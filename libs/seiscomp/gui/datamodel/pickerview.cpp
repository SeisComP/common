...
void PickerView::init() {
	...
	*/
	...
	if ( pickers ) {
		...
		auto *spickers = Processing::SecondaryPickerFactory::Services();
		if ( spickers ) {
			if ( !spickers->empty() ) {
				sort(spickers->begin(), spickers->end());
...
void PickerView::automaticRepick() {
	...
	if ( !SC_D.currentRecord->cursorText().isEmpty() ) {
		...
		if ( picker ) {
			...
			auto count = feedRepickerComponents(picker.get());
			if ( count < 0 ) {
				return;
			}
		...
		auto *rowWidget = SC_D.recordView->currentItem()->widget();
		...
		for ( int i = 0; i < rowWidget->markerCount(); ++i ) {
			...
			if ( !m->text().trimmed().startsWith('P') ) {
				continue;
			}
		...
		auto count = feedRepickerComponents(spicker.get());
		if ( count < 0 ) {
			return;
		}
