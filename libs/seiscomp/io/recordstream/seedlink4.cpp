...
bool SL4Connection<SocketType>::setSource(const string &source) {
	...
	if ( url.hasQueryItem("timeout") ) {
		unsigned int seconds;

		if ( Core::fromString(seconds, url.queryItemValue("timeout")) ) {
			_sock.setTimeout(seconds);
		}
		else {
			SEISCOMP_ERROR("Invalid 'timeout' (must be unsigned integer)");
			return false;
		}
	}

	if ( url.hasQueryItem("retries") ) {
		if ( !Core::fromString(_maxRetries, url.queryItemValue("retries")) ) {
			SEISCOMP_ERROR("Invalid 'retries' (must be integer)");
			return false;
		}
	}
	...
	if ( url.hasQueryItem("fetch") ) {
		if ( !Core::fromString(_fetch, url.queryItemValue("fetch")) ) {
			SEISCOMP_ERROR("Invalid 'fetch' (must be 'true' or 'false')" );
			return false;
		}
	}
	...
	if ( url.port() == Core::None ) {
		_serverloc += string(":") + DefaultSL4Port<SocketType>;
	}
	else {
		_serverloc += string(":") + toString(url.port());
	}
...
Record *SL4Connection<SocketType>::next() {
	...
	while ( !_sock.isInterrupted() ) {
		try {
			...
			string stationId(_slrecord.data() + HEADSIZE, head->stationIdLength);
			_stationSeq[stationId] = head->seq + 1;
