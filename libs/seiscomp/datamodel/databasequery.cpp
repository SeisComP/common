...
DatabaseIterator DatabaseQuery::getEventPickIDs(const std::string& publicID) {
	...
	return getObjectIterator(query, nullptr);
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
