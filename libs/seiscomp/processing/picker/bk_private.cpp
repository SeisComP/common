...
int preset(double *rbuf, int *n, double *old, double *y2,
           double *yt, double *sumx, double *sumx2, double *sdev, int *nsum,
           int *itar, int *ptime, int *preptime, char *pfm, int *ipkflg,
...
int ppick(double *reltrc, double *trace, int npts, double *
	...
	int realdown, i;
	double y2, yt;
	int picklength, amp, num;
	double sum, ssx, rda2, rdi2, ssx2, edat, mean, edev, rdif;
	int iamp;
	double rdat;
	int itar;
	double sdev;
	int itrm = -1;
	double omega;
	int dtime = 0, p_dur, realup;
	double rawold;

	int nstart, end_dur, amptime;
	...
	*signaltime = 0;
	...
	if ( i > npts ) {
		...
		*signaltime = amptime;
			...
			if ( itrm >= *tupevent ) {
				...
				if ( *ptime == 0 ) {
					*ptime = itar;
					...
					if ( *test != 0 ) {
						std::cerr <<  "pick at sample: " << *ptime << std::endl;
						std::cerr <<   " uptime= " << itrm << std::endl;
		...
		if ( *test != 0 ) {
			std::cerr << "pick at sample: " << *ptime    << std::endl;
			std::cerr << " uptime=      "   <<  itrm     << std::endl; // TODO: is this correct???
			std::cerr << " tupevent=    "   << *tupevent << std::endl;
			std::cerr << " dire=        "   << *ifrst    << std::endl;
			std::cerr << " Firstmotion= "   <<  *pfm     << std::endl;
		}
		...
		if ( *ipkflg != 0 ) {
			...
			if ( dtime > *tdownmax ) {
				...
				if ( realup >= *tupevent ) {
					...
					if ( *ptime == 0 ) {
						*ptime = itar;
						if ( *samplespersec < 100 ) {
							// Hack for old AD converter at the SED, no longer in use
							if ( *ifrst < 0 ) {
								pfm[0]='U';
							}
							/* ad-wandler invertiert */
