...
void AmplitudeProcessor::computeTimeWindow() {
	...
	if ( static_cast<double>(_config.noiseBegin) > static_cast<double>(_config.noiseEnd) ) {
		SEISCOMP_INFO("Invalid time window: noiseBegin (%f) after noiseEnd (%f)",
	...
	if ( static_cast<double>(_config.signalBegin) > static_cast<double>(_config.signalEnd) ) {
		SEISCOMP_INFO("Invalid time window: signalBegin (%f) after signalEnd (%f)",
	...
	if ( static_cast<double>(_config.noiseBegin) >=static_cast<double>(_config.signalEnd) ) {
		SEISCOMP_INFO("Zero overall time window: noiseBegin (%f) equals signalEnd (%f)",
		              static_cast<double>(_config.noiseBegin),
		              static_cast<double>(_config.signalEnd));
		setStatus(Error, 543);
		setTimeWindow(Core::TimeWindow());
	...
	if ( static_cast<double>(_config.noiseBegin) > static_cast<double>(_config.signalEnd) ) {
		SEISCOMP_INFO("Invalid overall time window: noiseBegin (%f) after signalEnd (%f)",
		              static_cast<double>(_config.noiseBegin),
		              static_cast<double>(_config.signalEnd));
		setStatus(Error, 544);
		setTimeWindow(Core::TimeWindow());
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
std::string AmplitudeProcessor::parameter(Capability cap) const {
	return std::string();
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
