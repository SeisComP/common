...
using Cat2Colors = map<Parts, ColorList>;
using Cat1Colors = map<Category, Cat2Colors>;
using Themes = array<Cat1Colors, 16>;
using Shapes = array<array<string, PartsQuantity>, 16>;
...
string multiavatar(string something, bool withoutEnv) {
	static auto themes = initThemes();
	static auto shapes = initShapes();

	auto svgStart = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 231 231\">";
	auto svgEnd = "</svg>";
	...
	if ( something.empty() ) {
		...
		uniform_int_distribution<> distrib(0, 47);
		h = 0;
		...
		h = hash<string>{}(something);
		// 34 bits ~ 48**6
		// Add the above 30 bits to the lower 34 bits
		h += (h >> 30);
	}
	...
	std::array<Category, 6> theme;
	std::array<string, 6> final;
	...
	for ( size_t i = 0; i < 6; ++i ) {
		auto partV = parts[i];
		auto th = theme[i];
		const auto &colors = themes[partV][th][Parts(i)];
		const auto &svgPart = shapes[partV][Parts(i)];

		size_t p0, p1 = 0, si = 0;
		size_t ci = 0;
		// cerr << "-----" << endl;
		while ( (p0 = svgPart.find('#', p1)) != string::npos ) {
			p1 = svgPart.find(';', p0 + 1);
			// cerr << ci << "/" << colors.size() << " " << svgPart.substr(p0, p1 - p0) << endl;
			final[i] += string_view(svgPart.data() + si, p0 - si);
			final[i] += colors[ci];
			si = p1;
			++p1;
			++ci;
		...
		if ( si < svgPart.size() ) {
			final[i]+= string_view(svgPart.data() + si, svgPart.size() - si);
		}
	}
