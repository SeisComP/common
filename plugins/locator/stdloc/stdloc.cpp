...
std::vector<std::string> splitString(const std::string &str,
  ...
  boost::split(tokens, str, boost::is_any_of(split), boost::token_compress_off);
  return tokens;
}
...
double computeCircularMean(const std::vector<double> &angles, bool useRadiant) {
  double y = 0, x = 0;
  for (size_t i = 0; i < angles.size(); i++) {
    double angle = useRadiant ? angles[i] : degToRad(angles[i]);
    x += cos(angle);
    y += sin(angle);
  }
  double mean = atan2(y / angles.size(), x / angles.size());
  return useRadiant ? mean : radToDeg(mean);
}
...
double computeDistance(double lat1, double lon1, double depth1, double lat2,
  ...
  double Vdist = std::abs(depth1 - depth2);
  ...
  for (const string &profileName : profileNames) {
    ...
    try {
      ...
      if (method == "LeastSquares") {
        ...
        SEISCOMP_ERROR("Profile %s: unrecognized method %s", prof.name.c_str(),
                       method.c_str());
        return false;
      }
    } catch (...) {
    ...
    try {
      prof.tttType = config.getString(prefix + "tableType");
    } catch (...) {
    }
    ...
    try {
      prof.gridSearch.errorType =
          config.getString(prefix + "GridSearch.errorType");
...
IDList StdLoc::profiles() const {
  ...
  std::transform(
      ...
      [](decltype(_profiles)::value_type const &pair) { return pair.first; });
  return keys;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
Origin *StdLoc::locate(PickList &pickList) {
  ...
  if (_currentProfile.method != Profile::Method::GridSearch &&
    ...
    throw LocatorException(
        "Cannot locate without an initial location: add the GridSearch");
  }
...
void StdLoc::computeAdditionlPickInfo(const PickList &pickList,
  ...
  for (size_t i = 0; i < pickList.size(); ++i) {
    ...
    if (_currentProfile.usePickUncertainties) {
      double uncertainty =
          getPickUncertainty(pick.get(), _currentProfile.defaultTimeError);
...
void StdLoc::locateGridSearch(
  ...
  if (_currentProfile.gridSearch.autoLatLon) {
    ...
    gridOriginLon = normalizeLon(computeCircularMean(sensorLon, false));
  ...
  for (double x = -_currentProfile.gridSearch.xExtent / 2. +
    ...
    for (double y = -_currentProfile.gridSearch.yExtent / 2. +
      ...
      for (double z = -_currentProfile.gridSearch.zExtent / 2. +
        ...
        double distance = std::sqrt(y * y + x * x); // km
        double azimuth = radToDeg(std::atan2(y, x));
        ...
        SEISCOMP_DEBUG(
            "Processing cell x=%f y=%f z=%f - lat %.6f lon %.6f depth %.3f", x,
            y, z, cellLat, cellLon, cellDepth);
        ...
        if (!std::isfinite(lowestError) || lowestError > error) {
          lowestError = error;
          newLat = cellLat;
          newLon = cellLon;
          newDepth = cellDepth;
...
void StdLoc::locateLeastSquares(
  ...
  for (int iteration = 0; iteration < _currentProfile.leastSquare.iterations;
    ...
    if (!std::isfinite(xCorrection) || !std::isfinite(yCorrection) ||
        !std::isfinite(zCorrection) || !std::isfinite(timeCorrection)) {
    ...
    double distance =
        std::sqrt(xCorrection * xCorrection + yCorrection * yCorrection); // km
    double azimuth = radToDeg(std::atan2(yCorrection, xCorrection));
...
Origin *StdLoc::createOrigin(
    ...
    const Core::Time &originTime) const {
  ...
  for (size_t i = 0; i < pickList.size(); ++i) {
    ...
    double azimuth = 0;
    double distance =
        computeDistance(originLat, originLon, originDepth, sensorLat[i],
                        sensorLon[i], -sensorElev[i] / 1000, &azimuth);
  ...
  oq.setMinimumDistance(*std::min_element(distances.begin(), distances.end()));
  oq.setMaximumDistance(*std::max_element(distances.begin(), distances.end()));
